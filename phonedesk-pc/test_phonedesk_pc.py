...
adb-ABC123._adb-tls-pairing._tcp. 192.168.1.55:37123
adb-ABC123._adb-tls-connect._tcp. 192.168.1.55:40421
"""
        self.assertEqual(
            phonedesk_pc.parse_mdns_services(output),
            {"pairing": ("192.168.1.55", 37123), "connect": ("192.168.1.55", 40421)},
        )

    def test_parse_mdns_services_accepts_service_names_without_trailing_dot(self):
        output = "adb-X _adb-tls-pairing._tcp 10.0.0.5:37899"
        found = phonedesk_pc.parse_mdns_services(output)
        self.assertEqual(found["pairing"], ("10.0.0.5", 37899))
